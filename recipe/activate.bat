...
reg query "HKLM\SOFTWARE\ODBC\ODBCINST.INI\ODBC Drivers" /v "ODBC Driver %MAJOR_VERSION% for SQL Server" >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo Backing up existing registry entries...
    reg export "HKLM\SOFTWARE\ODBC\ODBCINST.INI\ODBC Driver %MAJOR_VERSION% for SQL Server" "%REG_BACKUP_DIR%\driver_entry.reg" /y >nul 2>&1
...
set "FULL_DRIVER_PATH=%FULL_PATH%\Library\bin\msodbcsql%MAJOR_VERSION%.dll"
...
:: ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
reg add "HKLM\SOFTWARE\ODBC\ODBCINST.INI\ODBC Drivers" /v "ODBC Driver %MAJOR_VERSION% for SQL Server" /t REG_SZ /d "Installed" /f >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    reg add "HKLM\SOFTWARE\ODBC\ODBCINST.INI\ODBC Driver %MAJOR_VERSION% for SQL Server" /v Driver /t REG_SZ /d "%FULL_DRIVER_PATH%" /f >nul 2>&1
    reg add "HKLM\SOFTWARE\ODBC\ODBCINST.INI\ODBC Driver %MAJOR_VERSION% for SQL Server" /v Description /t REG_SZ /d "Microsoft ODBC Driver %MAJOR_VERSION% for SQL Server" /f >nul 2>&1
    reg add "HKLM\SOFTWARE\ODBC\ODBCINST.INI\ODBC Driver %MAJOR_VERSION% for SQL Server" /v Threading /t REG_SZ /d "1" /f >nul 2>&1
    reg add "HKLM\SOFTWARE\ODBC\ODBCINST.INI\ODBC Driver %MAJOR_VERSION% for SQL Server" /v UsageCount /t REG_DWORD /d 1 /f >nul 2>&1
