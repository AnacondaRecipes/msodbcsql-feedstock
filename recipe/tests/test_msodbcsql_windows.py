#!/usr/bin/env python
# test_msodbcsql_windows_pytest.py - pytest tests for msodbcsql on Windows
"""
...
def setup_environment():
    ...
    driver_path = os.path.join(conda_prefix, 'Library', 'bin', f'msodbcsql{msodbcsql_major_version}.dll')
...
def test_additional_dll_files_exist():
    ...
    print(f"Checking msodbcsqlr{msodbcsql_major_version}.rll existence: {resource_path}")
    assert os.path.exists(resource_path), f"msodbcsqlr{msodbcsql_major_version}.rll not found at path: {resource_path}"

def test_sdk_files_exist():
    include_dir = os.path.join(conda_prefix, 'Library', 'include', f'msodbcsql{msodbcsql_major_version}')
...
def test_driver_registered_in_odbcinst_ini():
    ...
    assert f"msodbcsql{msodbcsql_major_version}.dll" in content, "Incorrect driver path in odbcinst.ini"
...
def test_windows_registry_entries():
    ...
    try:
        ...
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, r'SOFTWARE\ODBC\ODBCINST.INI\ODBC Drivers') as key:
            driver_value = winreg.QueryValueEx(key, f'ODBC Driver {msodbcsql_major_version} for SQL Server')[0]
            assert driver_value == 'Installed', "Driver not marked as installed in HKLM registry"
            print("Driver registered in HKLM registry")
        
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, rf'SOFTWARE\ODBC\ODBCINST.INI\ODBC Driver {msodbcsql_major_version} for SQL Server') as key:
            driver_path_value = winreg.QueryValueEx(key, 'Driver')[0]
            assert f'msodbcsql{msodbcsql_major_version}.dll' in driver_path_value, "Incorrect driver path in HKLM registry"
